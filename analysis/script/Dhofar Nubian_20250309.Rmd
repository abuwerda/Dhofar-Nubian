...
df = read_csv("DN_Database_20250309.csv")
df_filtered <- df %>%
  filter() %>%
...
_Table S2: Nub core descriptive statistics by site__
```{r}
...
```
_Table S3: Nub core descriptive statistics by cluster__
```{r}
...
__Table S4: Bi core descriptive statistics by site__
```{r}
df_bi <- df_filtered %>%
  filter(Art_Type == "bi")
...
kable(descriptive_stats_thickness_site, caption = "Descriptive Statistics for Max Thickness by Site")
```
__Table S5: Bi core descriptive statistics by cluster__
```{r}
...
__Table S6: Uni core descriptive statistics by site__
```{r}
df_uni <- df_filtered %>%
...
kable(descriptive_stats_thickness_site, caption = "Descriptive Statistics for Max Thickness by Site")
```
__Table S7: Uni core descriptive statistics by cluster__
```{r}
...
__Table S8: Nubian point  descriptive statistics by site__
```{r}
...
kable(descriptive_stats_thickness_site, caption = "Descriptive Statistics for Max Thickness by Site")
```
__Table S9: Nubian point descriptive statistics by cluster__
```{r}
...
### Nubian Point Metrics

__Figure S72: Boxplot of Nubian point length by site__
```{r}
...
```
__Figure S73: Boxplot of Nubian point length by cluster__
```{r}
...
```
__Kruskal Wallis and Dunn's test of Nubian point length__
```{r}
...
```
__Figure S74: Boxplot of Nubian point width by site__
```{r}
...
```
__Figure S75: Boxplot of Nubian point width by cluster__
```{r}
...
```
__Figure S77: Boxplot of Nubian point thickness by cluster__
```{r}
...
```
__Figure S79: Boxplot of Nubian point Index of Flattening by cluster__
```{r}
...
```
__Figure S81: Boxplot of Nubian point Index of Elongation by cluster__
```{r}
...
```
__Kruskal Wallis and Dunn's Tests of Nubian point IE__
```{r}
...
```
__Figure S82: Nubian point L x W scatterplot by cluster__
```{r}
slopes <- df_filtered %>%
...
```
__Figure S83: Boxplot of Nubian point geometric mean by site__
```{r}
...
```
__Figure 15: Boxplot of Nubian point geometric mean by cluster__
```{r}
...
```
__Kruskal Wallis & Dunn's tests of Nubian point geo mean__
```{r}
...
__Figure S84: Boxplot of Nubian point TCSA by site__
```{r}
...
```
__Figure S85: Boxplot of Nubian point TCSA by cluster__
```{r}
...
```
__Figure S87: Boxplot of Nubian point TCSP by cluster__
```{r}
...
```
__Kruskal Wallis test of Nubian point TCSP__
```{r}
...
```
### Comparison of Nubian points versus preferential scars
...
```
__Kruskal Wallis test of Nubian point versus scar length__ 
```{r}
df_combined <- df_filtered %>%
  filter((!is.na(Lev_Scar_Length) & Art_Type == "Lev_Nub") | 
         (!is.na(Max_Length) & Art_Type_Sub == "point_Lev")) %>%
...
__Table S10: Data table of tool types by site__ 
```{r}
result <- df_filtered %>%
...
kable(pivot_result, caption = "Tool Types by Site")
```
...
pivot_result_cluster
kable(pivot_result_cluster, caption = "Tool Types by Cluster")
```
