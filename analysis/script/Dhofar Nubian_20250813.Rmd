---
title: "Analysis of Dhofar MSA/MP Assemblages"
date: "`r Sys.Date()`"
output:
  rmdformats::robobook:
    highlight: kate
...
set.seed(132)
# Set your working directory and make sure to include within it the raw data file "DN_Database_20250801.csv" In order for the ggsave function to work, you must also have a sub-folder in your working directory called "Figures," and within the Figures directory you will need two sub-folders called "MS" (which stores figures from the main text) and "SI" (which stores figures from the supplementary information file).
...
df = read_csv("DN_Database_20250813.csv")
df_filtered <- df %>%
  filter() %>%
...
kable(pivot_result_cluster, caption = "")
```
...
```
__Figure S4: Ratio of Levallois products to Nubian Levallois cores__
```{r}
artifact_counts <- df_filtered %>%
  group_by(Site, Cluster_3) %>%
  summarize(
...
lev_point_ratio <- ggplot(artifact_counts, aes(x = Site, y = point_lev_to_lev_nub_ratio, fill = as.factor(Cluster_3))) +
  ...
  labs(
       x = "",
       y = "Levallois products /  Nubian Levallois cores",
       fill = "Cluster") +
  theme(
    axis.text.x = element_text(size = 17, angle = 45, hjust = 1, face = "bold"),
    axis.title.y = element_text(size = 17),
...
core_type_patination <- ggplot(percentage_table, aes(x = factor(Patination), y = percentage, fill = Art_Type)) +
  ...
  scale_fill_manual(
    ...
    labels = c(
      "bi" = "bidirectional",
      "Kombewa" = "Kombewa",
      "Lev_cent" = "Levallois, centripetal",
      "Lev_conv" = "Levallois, convergent",
      "Lev_indeterm" = "Levallois, indeterminate",
      "Lev_Nub" = "Levallois, Nubian",
...
__Table S1: Nub core descriptive statistics by site__
```{r}
...
```
__Table S2: Nub core descriptive statistics by cluster__
```{r}
...
```
__Figure 11: Bi core dorsal scar pie charts by cluster__
```{r}
df_bi_ds <- df_filtered %>%
  filter(Art_Type == "bi", !is.na(Scar_Pattern))
data_counts <- df_bi_ds %>%
...
bi_ds_cluster
ggsave("Figures/MS/11-bi_core_ds_cluster.png", plot = bi_ds_cluster, width = 12, height = 8)
```
...
print(posthoc_results_fisher)
```
__Figure S33: Bi core platform prep pie charts for all sites__
```{r}
df_bi_p <- df_filtered %>%
...
bi_plat_prep_site
ggsave("Figures/SI/S33-bi_plat_prep_site.png", plot = bi_plat_prep_site, width = 12, height = 8)
```
__Figure S34: Bi core platform prep pie charts by cluster__
```{r}
df_bi_p <- df_filtered %>%
  filter(Art_Type == "bi", Primary_Plat != "crushed&missing&indeterm", !is.na(Primary_Plat))
data_counts <- df_bi_p %>%
...
bi_core_plat
ggsave("Figures/SI/S34-bi_core_plat.png", plot = bi_core_plat, width = 12, height = 8)
```
...
print(chisq_test)
```
__Figure S35: Bi core shape pie charts for all sites__
```{r}
df_bi_s <- df_filtered %>%
...
bi_shape_sites
ggsave("Figures/SI/S35-bi_shape_sites.png", plot = bi_shape_sites, width = 12, height = 8)
```
__Figure S36: Bi core shape pie charts by cluster__
```{r}
df_bi_s <- df_filtered %>%
...
bi_core_shape
ggsave("Figures/SI/S36-bi_core_shape_cluster.png", plot = bi_core_shape, width = 12, height = 8)
```
...
print(posthoc_results_fisher)
```
__Figure S37: Bi core final scar pie charts for all sites__
```{r}
...
bi_fls_sites
ggsave("Figures/SI/S37-bi_fls_sites.png", plot = bi_fls_sites, width = 12, height = 8)
```
__Figure S38: Bi core final scar pie charts by cluster__
```{r}
df_bi_fls <- df_filtered %>%
  filter(Art_Type == "Lev_Nub", !is.na(Lev_Scar), Lev_Scar != "recycled", Lev_Scar != "overpassed")
data_counts <- df_bi_fls %>%
...
Nub_core_fls_cluster
ggsave("Figures/SI/S38-nub_core_fls_cluster.png", plot = Nub_core_fls_cluster, width = 12, height = 8)
```
...
__Figure S39: Boxplot of bi core length by site__
```{r}
...
bi_core_length_site
ggsave("Figures/SI/S39-bi_core_length_site.png", plot = bi_core_length_site, width = 12, height = 8)
```
__Figure S40: Boxplot of Nub core length by cluster__
```{r}
...
ggsave("Figures/SI/S40-bi_core_length_cluster.png", plot = bi_core_length_cluster, width = 12, height = 8)
```
...
```
__Figure S41: Boxplot of bi core width by site__
```{r}
...
bi_core_width_site
ggsave("Figures/SI/S41-bi_core_width_site.png", plot = bi_core_width_site, width = 12, height = 8)
```
__Figure S42: Boxplot of bi core width by cluster__
```{r}
...
ggsave("Figures/SI/S42-bi_core_width_cluster.png", plot = bi_core_width_cluster, width = 12, height = 8)
```
...
```
__Figure S43: Boxplot of bi core thickness by site__
```{r}
...
bi_core_thick_site
ggsave("Figures/SI/S43-bi_core_thick_site.png", plot = bi_core_thick_site, width = 12, height = 8)
```
__Figure S44: Boxplot of bi core thickness by cluster__
```{r}
...
ggsave("Figures/SI/S44-bi_core_thick_cluster.png", plot = bi_core_thick_cluster, width = 12, height = 8)
```
...
```
__Figure S45: Boxplot of bi core Index of Flattening by site__
```{r}
...
bi_core_IF_site
ggsave("Figures/SI/S45-bi_core_IF_site.png", plot = bi_core_IF_site, width = 12, height = 8)
```
__Figure S46: Boxplot of bi core Index of Flattening by cluster__
```{r}
...
bi_core_IF_cluster
ggsave("Figures/SI/S46-bi_core_IF_cluster.png", plot = bi_core_IF_cluster, width = 12, height = 8)
```
...
```
__Figure S47: Boxplot of bi core Index of Elongation by site__
```{r}
...
bi_core_IE_site
ggsave("Figures/SI/S47-bi_core_IE_site.png", plot = bi_core_IE_site, width = 12, height = 8)
```
__Figure S48: Boxplot of bi core Index of Elongation by cluster__
```{r}
...
bi_core_IE_cluster
ggsave("Figures/SI/S48-bi_core_IE_cluster.png", plot = bi_core_IE_cluster, width = 12, height = 8)
```
...
```
__Figure S49: bi core L x W scatterplot by cluster__
```{r}
slopes <- df_filtered %>%
...
bi_core_LW_scatter
ggsave("Figures/SI/S49-bi_core_LW_scatter.png", plot = bi_core_LW_scatter, width = 12, height = 8)
```
__Figure 12: Boxplot of bi core geometric mean by site__
```{r}
...
ggsave("Figures/MS/12-bi_core_geo_mean_site.png", plot = bi_core_geo_mean_site, width = 12, height = 8)
```
__Figure S50: Boxplot of bi core geometric mean by cluster__
```{r}
...
ggsave("Figures/SI/S50-bi_core_geo_mean_cluster.png", plot = bi_core_geo_mean_cluster, width = 12, height = 8)
```
...
__Figure S51: Uni core dorsal scar pie charts for all sites__
```{r}
...
uni_ds_sites
ggsave("Figures/SI/S51-uni_ds_sites.png", plot = uni_ds_sites, width = 12, height = 8)
```
__Figure 14: Uni core dorsal scar pie charts by cluster__
```{r}
df_uni_ds <- df_filtered %>%
  filter(Art_Type %in% c("uni_blade", "uni_flake"), !is.na(Scar_Pattern))
data_counts <- df_uni_ds %>%
...
uni_ds_cluster
ggsave("Figures/MS/14-uni_core_ds_cluster.png", plot = uni_ds_cluster, width = 12, height = 8)
```
...
print(posthoc_results_fisher)
```
__Figure S52: Uni core platform prep pie charts for all sites__
```{r}
...
uni_plat_prep_site
ggsave("Figures/SI/S52-uni_plat_prep_site.png", plot = uni_plat_prep_site, width = 12, height = 8)
```
__Figure S53: Uni core platform prep pie charts by cluster__
```{r}
df_uni_p <- df_filtered %>%
  filter(Art_Type %in% c("uni_blade", "uni_flake"), Primary_Plat != "crushed&missing&indeterm", !is.na(Primary_Plat))
data_counts <- df_uni_p %>%
...
ggsave("Figures/SI/S53-uni_core_plat_cluster.png", plot = uni_core_plat_cluster, width = 12, height = 8)
```
__Chi Square Test of Platform differences between clusters__
```{r}
df_uni_p <- df_filtered %>%
...
chisq_test
```
__Figure S54: Uni core shape pie charts for all sites__
```{r}
...
uni_shape_sites
ggsave("Figures/SI/S54-uni_shape_sites.png", plot = uni_shape_sites, width = 12, height = 8)
```
__Figure 15: uni core shape pie charts by cluster__
```{r}
df_uni_s <- df_filtered %>%
...
uni_core_shape
ggsave("Figures/MS/15-uni_core_shape.png", plot = uni_core_shape, width = 12, height = 8)
```
__Chi Square Test of shape differences between clusters__
```{r}
df_uni_s <- df_filtered %>%
...
__Figure S55: Boxplot of uni core length by site__
```{r}
...
ggsave("Figures/SI/S55-uni_core_length_site.png", plot = uni_core_length_site, width = 12, height = 8)
```
__Figure S56: Boxplot of uni core length by cluster__
```{r}
...
ggsave("Figures/SI/S56-uni_core_length_cluster.png", plot = uni_core_length_cluster, width = 12, height = 8)
```
...
```
__Figure S57: Boxplot of uni core width by site__
```{r}
...
uni_core_width_site
ggsave("Figures/SI/S57-uni_core_width_site.png", plot = uni_core_width_site, width = 12, height = 8)
```
__Figure S58: Boxplot of uni core width by cluster__
```{r}
...
ggsave("Figures/SI/S58-uni_core_width_cluster.png", plot = uni_core_width_cluster, width = 12, height = 8)
```
...
```
__Figure S59: Boxplot of uni core thickness by site__
```{r}
...
uni_core_thick_site
ggsave("Figures/SI/S59-uni_core_thick_site.png", plot = uni_core_thick_site, width = 12, height = 8)
```
__Figure S60: Boxplot of uni core thickness by cluster__
```{r}
...
ggsave("Figures/SI/S60-uni_core_thick_cluster.png", plot = uni_core_thick_cluster, width = 12, height = 8)
```
...
```
__Figure S61: Boxplot of uni core Index of Flattening by site__
```{r}
...
uni_core_IF_site
ggsave("Figures/SI/S61-uni_core_IF_site.png", plot = uni_core_IF_site, width = 12, height = 8)
```
__Figure S62: Boxplot of uni core Index of Flattening by cluster__
```{r}
...
uni_core_IF_cluster
ggsave("Figures/SI/S62-uni_core_IF_cluster.png", plot = uni_core_IF_cluster, width = 12, height = 8)
```
...
```
__Figure S63: Boxplot of uni core Index of Elongation by site__
```{r}
...
uni_core_IE_site
ggsave("Figures/SI/S63-uni_core_IE_site.png", plot = uni_core_IE_site, width = 12, height = 8)
```
__Figure S64: Boxplot of uni core Index of Elongation by cluster__
```{r}
...
uni_core_IE_cluster
ggsave("Figures/SI/S64-uni_core_IE_cluster.png", plot = uni_core_IE_cluster, width = 12, height = 8)
```
...
```
__Figure S65: uni core L x W scatterplot by cluster__
```{r}
slopes <- df_filtered %>%
...
uni_core_LW_scatter
ggsave("Figures/SI/S65-uni_core_LW_scatter.png", plot = uni_core_LW_scatter, width = 12, height = 8)
```
__Figure 16: Boxplot of uni core geometric mean by site__
```{r}
...
ggsave("Figures/MS/16-uni_core_geo_mean_site.png", plot = uni_core_geo_mean_site, width = 12, height = 8)
```
__Figure S66: Boxplot of uni core geometric mean by cluster__
```{r}
...
ggsave("Figures/SI/S66-uni_core_geo_mean_cluster.png", plot = uni_core_geo_mean_cluster, width = 12, height = 8)
```
...
__Figure S67: debitage L x W scatterplot by site__
```{r}
slopes <- df_filtered %>%
...
ggsave("Figures/SI/S67-debitage_LW_scatter_site.png", plot = debitage_LW_scatter_site, width = 12, height = 8)
```
__Figure S68: debitage L x W scatterplot by cluster__
```{r}
slopes <- df_filtered %>%
    filter(Art_Class == "debitage", !is.na(Max_Length), !is.na(Max_Width)) %>%
    group_by(Cluster_3) %>%
...
ggsave("Figures/SI/S68-debitage_LW_scatter_cluster.png", plot = debitage_LW_scatter_cluster, width = 12, height = 8)
```
...
__Figure S69: Boxplot of Levallois point length by site__
```{r}
...
lp_length_site
ggsave("Figures/SI/S69-lp_length_site.png", plot = lp_length_site, width = 12, height = 8)
```
__Figure S70: Boxplot of Levallois point length by cluster__
```{r}
...
lp_length_cluster
ggsave("Figures/SI/S70-lp_length_cluster.png", plot = lp_length_cluster, width = 12, height = 8)
```
...
```
__Figure S71: Boxplot of Levallois point width by site__
```{r}
...
lp_width_site
ggsave("Figures/SI/S71-lp_width_site.png", plot = lp_width_site, width = 12, height = 8)
```
__Figure S72: Boxplot of Levallois point width by cluster__
```{r}
...
lp_width_cluster
ggsave("Figures/SI/S72-lp_width_cluster.png", plot = lp_width_cluster, width = 12, height = 8)
```
...
```
__Figure S73: Boxplot of Levallois point thickness by site__
```{r}
...
lp_thick_site
ggsave("Figures/SI/S73-lp_thick_site.png", plot = lp_thick_site, width = 12, height = 8)
```
__Figure S74: Boxplot of Levallois point thickness by cluster__
```{r}
...
lp_thick_cluster
ggsave("Figures/SI/S74-lp_thick_cluster.png", plot = lp_thick_cluster, width = 12, height = 8)
```
...
```
__Figure S75: Boxplot of Levallois point Index of Flattening by site__
```{r}
...
lp_IF_site
ggsave("Figures/SI/S75-lp_IF_site.png", plot = lp_IF_site, width = 12, height = 8)
```
__Figure S76: Boxplot of Levallois point Index of Flattening by cluster__
```{r}
...
lp_IF_cluster
ggsave("Figures/SI/S76-lp_IF_cluster.png", plot = lp_IF_cluster, width = 12, height = 8)
```
...
```
__Figure S77: Boxplot of Levallois point Index of Elongation by site__
```{r}
...
lp_IE_site
ggsave("Figures/SI/S77-lp_IE_site.png", plot = lp_IE_site, width = 12, height = 8)
```
__Figure S78: Boxplot of Levallois point Index of Elongation by cluster__
```{r}
...
lp_IE_cluster
ggsave("Figures/SI/S78-lp_IE_cluster.png", plot = lp_IE_cluster, width = 12, height = 8)
```
...
```
__Figure S79: Levallois point L x W scatterplot by cluster__
```{r}
slopes <- df_filtered %>%
...
lp_LW_scatter
ggsave("Figures/SI/S79-lp_LW_scatter.png", plot = lp_LW_scatter, width = 12, height = 8)
```
__Figure S80: Boxplot of Levallois point geometric mean by site__
```{r}
...
lp_geo_mean_site
ggsave("Figures/SI/S80-lp_geo_mean_site.png", plot = lp_geo_mean_site, width = 12, height = 8)
```
__Figure 18: Boxplot of Levallois point geometric mean by cluster__
```{r}
...
lp_geo_mean_cluster
ggsave("Figures/MS/18-lp_geo_mean_cluster.png", plot = lp_geo_mean_cluster, width = 12, height = 8)
```
...
```
__Figure S81: Boxplot of Levallois point TCSA by site__
```{r}
...
lp_TCSA_site
ggsave("Figures/SI/S81-lp_TCSA_site.png", plot = lp_TCSA_site, width = 12, height = 8)
```
__Figure S82: Boxplot of Levallois point TCSA by cluster__
```{r}
...
lp_TCSA_cluster
ggsave("Figures/SI/S82-lp_TCSA_cluster.png", plot = lp_TCSA_cluster, width = 12, height = 8)
```
...
```
__Figure S83: Boxplot of Levallois point TCSP by site__
```{r}
...
lp_TCSP_site
ggsave("Figures/SI/S83-lp_TCSP_site.png", plot = lp_TCSP_site, width = 12, height = 8)
```
__Figure S84: Boxplot of Levallois point TCSP by cluster__
```{r}
...
lp_TCSP_cluster
ggsave("Figures/SI/S84-lp_TCSP_cluster.png", plot = lp_TCSP_cluster, width = 12, height = 8)
```
...
```
__Figure S85: Boxplot of preferential scar length by site__
```{r}
...
ls_length_site
ggsave("Figures/SI/S85-ls_length_site.png", plot = ls_length_site, width = 12, height = 8)
```
__Figure S86: Boxplot of preferential scar length by cluster__
```{r}
...
ls_length_cluster
ggsave("Figures/SI/S86-ls_length_cluster.png", plot = ls_length_cluster, width = 12, height = 8)
```
...
```
__Figure S87: Boxplot of preferential scar width by site__
```{r}
...
lp_width_site
ggsave("Figures/SI/S87-ls_width_site.png", plot = ls_width_site, width = 12, height = 8)
```
__Figure S88: Boxplot of preferential scar width by cluster__
```{r}
...
ls_width_cluster
ggsave("Figures/SI/S88-ls_width_cluster.png", plot = ls_width_cluster, width = 12, height = 8)
```
...
```
__Figure S89: Boxplot of preferential scar Index of Elongation by site__
```{r}
...
ls_IE_site
ggsave("Figures/SI/S89-ls_IE_site.png", plot = ls_IE_site, width = 12, height = 8)
```
__Figure S90: Boxplot of preferential scar Index of Elongation by cluster__
```{r}
...
ls_IE_cluster
ggsave("Figures/SI/S90-ls_IE_cluster.png", plot = ls_IE_cluster, width = 12, height = 8)
```
...
```
__Figure S91: Preferential scar L x W scatterplot by cluster__
```{r}
slopes <- df_filtered %>%
...
ls_LW_scatter
ggsave("Figures/SI/S91-ls_LW_scatter.png", plot = ls_LW_scatter, width = 12, height = 8)
```
__Figure S92: Boxplot of preferential scar size by site__
```{r}
...
ls_s_site
ggsave("Figures/SI/S92-ls_s_site.png", plot = ls_s_site, width = 12, height = 8)
```
__Figure S93: Boxplot of preferential scar size by cluster__
```{r}
...
ls_s_cluster
ggsave("Figures/SI/S93-ls_s_cluster.png", plot = ls_s_cluster, width = 12, height = 8)
```
...
__Figure S94: Boxplots of point vs. scar length by cluster__
```{r}
...
ggsave("Figures/SI/S94-point_vs_scar_length.png", plot = point_vs_scar_length, width = 12, height = 8)
```
...
```
__Figure S95: Boxplots of point vs. scar width by cluster__
```{r}
df_combined <- df_filtered %>%
...
ggsave("Figures/SI/S95-point_vs_scar_width.png", plot = point_vs_scar_width, width = 12, height = 8)
```
...
results$kruskal_test
```
__Figure 20: Boxplots of discarded point vs. scar size by cluster__
```{r}
df_combined <- df_filtered %>%
...
point_vs_scar_size <- ggplot(df_combined, aes(x = Variable, y = Value, fill = as.factor(Cluster_3))) +
  ...
  facet_grid(. ~ Cluster_3)
point_vs_scar_size
ggsave("Figures/MS/20-point_vs_scar_size.png", plot = point_vs_scar_size, width = 12, height = 8)
```
...
results$kruskal_test
```
__Figure 20b: Boxplots of point vs. scar size by cluster w/o TH.69__
```{r}

df_combined <- df_filtered %>%
  filter(Site != "TH.069" & 
         ((!is.na(Lev_Scar_Size) & Art_Type == "Lev_Nub") | 
          (!is.na(Size) & Art_Type_Sub == "point_Lev"))) %>%
  pivot_longer(cols = c(Lev_Scar_Size, Size), names_to = "Variable", values_to = "Value") %>%
  filter(!is.na(Value)) %>%
  mutate(Variable = case_when(
...
```
__Figure 20c: Boxplots of point vs. scar size at TH.69__
```{r}
df_combined <- df_filtered %>%
  filter(Site == "TH.069" & 
         ((!is.na(Lev_Scar_Size) & Art_Type == "Lev_Nub") | 
...
point_vs_scar_size_no69 <- ggplot(df_combined, aes(x = Variable, y = Value, fill = as.factor(Cluster_3))) +
  ...
  scale_fill_manual(values = rep("#1F78B4")) +
  theme_minimal() +
  theme(
    legend.position = "none",
    axis.text.x = element_text(hjust = 1, size = 13, angle = 45),
...
kable(pivot_result_cluster, caption = "")
```
__Figure S96: Bar plots of retouched tool counts per site (no Levallois products)__
```{r}
df_ret_tool_counts <- df_filtered %>%
  filter(Art_Class == "tool", !is.na(Patination), Tool_Type != "Levallois_blank")
...
ggsave("Figures/SI/S96-ret_tool_counts_site.png", plot = ret_tool_counts_site, width = 12, height = 8)
```
__Figure S97: Bar plot of retouched tool counts per cluster__
```{r}
df_ret_tool_counts <- df_filtered %>%
  filter(Art_Class == "tool", !is.na(Patination), Tool_Type != "Levallois_blank")
count_table <- df_ret_tool_counts %>%
...
ggsave("Figures/SI/S97-ret_tool_counts_cluster.png", plot = ret_tool_counts_cluster, width = 12, height = 8)
```

## Blank Selection

__Figure S98: Bar plots of retouched tool blank selection count per site__
```{r}
df_toolblankcounts <- df_filtered %>%
  filter(Art_Class == "tool", Art_Type != "indeterm", Tool_Type != "Levallois_blank")
...
ggsave("Figures/SI/S98-tool_blank_counts_site.png", plot = tool_blank_counts_site, width = 12, height = 8)
```
__Figure 19: Pie charts of retouched tool blank selection by cluster__
```{r}
...
ggsave("Figures/MS/19-ret_tool_blank_counts_cluster.png", plot = ret_tool_blank_counts_cluster, width = 12, height = 8)
```
...
__Figure S99: Boxplot of retouched tool length by site__
```{r}
...
ggsave("Figures/SI/S99-ret_tool_length_site.png", plot = ret_tool_length_site, width = 12, height = 8)
```
__Figure S100: Boxplot of retouched tool length by cluster__
```{r}
filtered_data <- df_filtered %>% 
  filter(Art_Class == "tool", !is.na(Max_Length), Tool_Type != "Levallois_blank", Art_Type != "biface")
...
ggsave("Figures/SI/S100-ret_tool_length_cluster.png", plot = ret_tool_length_cluster, width = 12, height = 8)
```
...
```
__Figure S101: Boxplot of retouched tool width by site__
```{r}
filtered_data <- df_filtered %>% 
...
ret_tool_width_site
ggsave("Figures/SI/S101-ret_tool_width_site.png", plot = ret_tool_width_site, width = 12, height = 8)
```
__Figure S102: Boxplot of retouched tool width by cluster__
```{r}
filtered_data <- df_filtered %>% 
  filter(Art_Class == "tool", !is.na(Max_Width), Tool_Type != "Levallois_blank", Art_Type != "biface")
...
ggsave("Figures/SI/S102-ret_tool_width_cluster.png", plot = ret_tool_width_cluster, width = 12, height = 8)
```
...
```
__Figure S103: Boxplot of retouched tool thickness by site__
```{r}
filtered_data <- df_filtered %>% 
...
ret_tool_thick_site
ggsave("Figures/SI/S103-ret_tool_thick_site.png", plot = ret_tool_thick_site, width = 12, height = 8)
```
__Figure S104: Boxplot of retouched tool thickness by cluster__
```{r}
filtered_data <- df_filtered %>% 
  filter(Art_Class == "tool", !is.na(Max_Thick), Tool_Type != "Levallois_blank", Art_Type != "biface")
...
ggsave("Figures/SI/S104-ret_tool_thick_cluster.png", plot = ret_tool_thick_cluster, width = 12, height = 8)
```
...
```
__Figure S105: Boxplot of retouched tool Index of Flattening by site__
```{r}
filtered_data <- df_filtered %>% 
...
ret_tool_IF_site
ggsave("Figures/SI/S105-ret_tool_IF_site.png", plot = ret_tool_IF_site, width = 12, height = 8)
```
__Figure S106: Boxplot of retouched tool Index of Flattening by cluster__
```{r}
filtered_data <- df_filtered %>% 
  filter(Art_Class == "tool", !is.na(Index_F), Tool_Type != "Levallois_blank", Art_Type != "biface")
...
ret_tool_IF_cluster
ggsave("Figures/SI/S106-ret_tool_IF_cluster.png", plot = ret_tool_IF_cluster, width = 12, height = 8)
```
...
```
__Figure S107: Boxplot of retouched tool Index of Elongation by site__
```{r}
filtered_data <- df_filtered %>% 
...
ret_tool_IE_site
ggsave("Figures/SI/S107-ret_tool_IE_site.png", plot = ret_tool_IE_site, width = 12, height = 8)
```
__Figure S108: Boxplot of retouched tool Index of Elongation by cluster__
```{r}
filtered_data <- df_filtered %>% 
  filter(Art_Class == "tool", !is.na(Index_E), Tool_Type != "Levallois_blank", Art_Type != "biface")
...
ret_tool_IE_cluster
ggsave("Figures/SI/S108-ret_tool_IE_cluster.png", plot = ret_tool_IE_cluster, width = 12, height = 8)
```
__Kruskal Wallis test of uni core IE__
```{r}
...
```
__Figure S109: retouched tool L x W scatterplot by cluster__
```{r}
slopes <- df_filtered %>%
...
ret_tool_LW_scatter
ggsave("Figures/SI/S109-ret_tool_LW_scatter.png", plot = ret_tool_LW_scatter, width = 12, height = 8)
```
__Figure S110: Boxplot of retouched tool geometric mean by site__
```{r}
filtered_data <- df_filtered %>% 
...
ret_tool_GM_site
ggsave("Figures/SI/S110-ret_tool_GM_site.png", plot = ret_tool_GM_site, width = 12, height = 8)
```
__Figure S111: Boxplot of retouched tool geometric mean by cluster__
```{r}
filtered_data <- df_filtered %>% 
  filter(Art_Class == "tool", !is.na(Geo_Mean), Tool_Type != "Levallois_blank", Art_Type != "biface")
...
ret_tool_GM_cluster
ggsave("Figures/SI/S111-ret_tool_GM_cluster.png", plot = ret_tool_GM_cluster, width = 12, height = 8)
```
